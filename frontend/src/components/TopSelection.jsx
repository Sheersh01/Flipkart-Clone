...
const items = [
  { img: hooks, name: 'WAIT2SHOP Hooks', subtitle: 'Most-loved' },
  { img: drawerDividers, name: 'Madrid Closet Drawer Dividers', subtitle: 'New Collection' },
  { img: surgicalGloves, name: 'Surgical Gloves', subtitle: 'Top Collection' },
  { img: kitchenTrolley, name: 'Kitchen Trolleys', subtitle: 'Special offer' },
];

function TopSelection() {
  return (
    <section className="top-selection" id="top-selection">
      <div className="container top-selection-inner" style={{ backgroundImage: `url(${bgImage})` }}>
        ...
        <div className="top-selection-grid">
          {items.map((item, i) => (
            <a href="#" className="top-selection-card" key={i} id={`top-selection-${i}`}>
              <div className="top-selection-img-wrapper">
                <img src={item.img} alt={item.name} className="top-selection-img" />
              </div>
